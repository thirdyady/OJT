...
-- Retain self-service reads, but all trainee attendance writes now pass through
-- the atomic RPC below. Existing admin correction policies remain in place.
DROP POLICY "own dtr all" ON public.dtr_entries;
CREATE POLICY "own dtr select" ON public.dtr_entries FOR SELECT TO authenticated
USING (auth.uid() = user_id AND public.dtr_is_active());

CREATE FUNCTION public.dtr_punch(
  action TEXT,
  expected_date DATE,
  expected_id UUID,
...
BEGIN
  ...
  END LOOP;
  IF undo THEN
    IF last_index IS DISTINCT FROM action_index THEN
      RAISE EXCEPTION 'Only the latest punch for today can be undone';
    END IF;
    stamps[action_index] := NULL;
  ELSE
